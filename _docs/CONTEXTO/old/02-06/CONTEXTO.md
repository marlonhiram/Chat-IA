...
- **Modelo de IA principal**: Gemini 2.5 Flash — via endpoint `/v1beta/`
- **Modelo de IA fallback**: Gemini 2.5 Flash Lite — ativado apenas na 3ª tentativa em caso de erro; se acionado, encaminha para Andreia via WhatsApp
...
```
Request → Facilitador (classifica intenção)
        ↓
Roteamento para agente correto:
  comercial    → Suh (Agente Comercial)
  compras      → Agente Compras
  rh           → Agente RH
...
**Gemini 2.5 Flash Lite (Google) — removido do fluxo**
- Não mais utilizado como fallback de modelo
- Erro técnico persistente → encaminhamento direto para Andreia via WhatsApp
...
- Botão [WHATSAPP]: link extraído da própria resposta via regex (`wa.me`); fallback para `montarLinkWhatsApp(msgs)` se nenhum link encontrado na resposta
...
│   ├── api/
│   │   ├── chat/
│   │   │   └── route.ts       (fluxo sequencial + retry/GeminiIndisponivelError + captura de tokens)
│   │   └── painel/
...
│   │   ├── facilitador.ts     (classificador com few-shot — tokens_max: 1024, temperatura: 0.1)
...
5. Facilitador classifica a mensagem (sequencial)
6. Facilitador retorna JSON limpo (markdown removido antes do parse)
7. Agente correto chamado conforme classificação
...
- Tentativa 1: `gemini-2.5-flash` — aguarda ~800ms±200 em caso de 503
- Tentativa 2: `gemini-2.5-flash` — aguarda ~1500ms±500 em caso de 503
- Tentativa 3: `gemini-2.5-flash` — se falhar, lança `GeminiIndisponivelError`
...
- Especificação customizada com intenção de compra: encaminha representante (não Andreia)
...
- Extrai link `wa.me` da resposta se presente (agente Compras → Josemar, erro técnico → Andreia)
- Fallback: `montarLinkWhatsApp(msgs)` com dados coletados da conversa (agente Comercial → Andreia/representante)
...
- **GeminiIndisponivelError sem modelo fallback**: decisão consciente de não usar Flash Lite como fallback de modelo — qualidade inferior incompatível com o fluxo complexo da Suh. Erro técnico persistente vai direto para atendimento humano.
